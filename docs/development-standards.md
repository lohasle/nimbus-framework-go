...
- 当前菜单未开放的页面不作为后端迁移清单。
- HTTP 状态 401 与响应体业务码 401 必须进入同一认证处理入口，页面组件不得各自弹出登录过期提示。
- 同一时刻只允许一次 Access Token 刷新；刷新失败必须立即清理本地令牌、用户缓存和动态路由，并跳转登录页。
- 失败请求最多自动重试一次。刷新失败或重试仍为 401 时不得回放请求队列，避免 401 风暴和循环刷新。
- 登录、注册、验证码及租户识别等公开接口须跳过会话刷新，业务校验失败只显示该接口自身错误。
